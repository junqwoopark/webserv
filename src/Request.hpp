...
class Request {
 public:
  ...
  void append(char *buffer, size_t size) {
    ...
    if (mMethod == "POST") {
      size_t clientMaxBodySize = 4096 * 1024;  // tmp
      if (mIsChunked) {
        mBuffer.append(buffer, size);
        while (true) {
          ...
          if (mBody.size() + chunkSize > clientMaxBodySize) {
            // error handle
            break;
          }
      ...
      //     size_t chunkStart = chunkEnd + 2;
      //     size_t chunkDataEnd = chunkStart + chunkSize;
      //     if (chunkDataEnd + 2 > mBuffer.size()) {  // chunk가 not completed
      //       break;
      //     }
      //     mBody.append(mBuffer.substr(chunkStart, chunkSize));
      //     mBuffer = mBuffer.substr(chunkDataEnd + 2);  // Move past the \r\n after the chunk data
      ...
      // }
        ...
        if (mBuffer.length() > clientMaxBodySize) {
          // error handle
        }
